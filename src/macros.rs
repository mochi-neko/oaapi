...
/// }
///
/// // impl_bool_enum_serialization!(TestEnum, A, B);
///
/// let serialized = serde_json::to_string(&TestEnum::A).unwrap();
/// assert_eq!(serialized, "true");
///
/// let deserialized: TestEnum = serde_json::from_str(&serialized).unwrap();
/// assert_eq!(deserialized, TestEnum::A);
///
/// let serialized = serde_json::to_string(&TestEnum::B).unwrap();
...
/// }
///
/// // impl_enum_with_string_or_array_serialization!(TestEnum, Single(String), Array(TestStruct));
///
/// assert_eq!(
///     serde_json::to_string(&TestEnum::Single("42".to_string())).unwrap(),
///     "\"42\""
/// );
///
/// assert_eq!(
///     serde_json::from_str::<TestEnum>("\"42\"").unwrap(),
///     TestEnum::Single("42".to_string())
/// );
///
/// assert_eq!(
///     serde_json::to_string(&TestEnum::Array(vec![TestStruct { value: 42 }])).unwrap(),
///     "[{\"value\":42}]"
/// );
///
/// assert_eq!(
///     serde_json::from_str::<TestEnum>("[{\"value\":42}]").unwrap(),
...
mod test {
    ...
    fn enum_struct_serialization() {
        ...
        let test = TestEnum::B(TestStructB::new(42));
        let serialized = serde_json::to_string(&test).unwrap();
        assert_eq!(
            serialized,
            "{\"tag\":\"b\",\"value\":42}"
        ...
        let test = TestEnum::C(TestStructC::new(42, 44));
        let serialized = serde_json::to_string(&test).unwrap();
        assert_eq!(
            serialized,
            "{\"tag\":\"c\",\"value\":42,\"other_value\":44}"
        );
        let deserialized: TestEnum = serde_json::from_str(&serialized).unwrap();
        assert_eq!(deserialized, test);
    }
    ...
    #[test]
    fn enum_struct_or_array_serialization() {
        #[derive(Debug, Clone, PartialEq)]
        enum TestEnum {
            Single(String),
            Array(Vec<TestStruct>),
